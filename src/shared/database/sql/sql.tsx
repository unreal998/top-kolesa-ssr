...
export async function getFilterData() {
    return new Promise( (resolve, reject) => 
    db.getConnection((err, connection) => {
        connection.execute(`SELECT DISTINCT width FROM ${`mod_tires_sizes`}`, function (err: string, result: TireSizesDataResponceDto[]) {
            if (err) {
                console.log('=====err===', err)
            }
            ...
            connection.execute(`SELECT DISTINCT diametr FROM ${`mod_tires_sizes`} ORDER BY diametr`, function (err: string, result: TireSizesDataResponceDto[]) {
                if (err) {
                    connection.release();
                    reject(err);
                }
                ...
                connection.execute(`SELECT DISTINCT height FROM ${`mod_tires_sizes`} ORDER BY height`, function (err: string, result: TireSizesDataResponceDto[]) {
                    if (err) {
                        connection.release();
                        reject(err);
                    }
                    ...
                    connection.execute(`SELECT DISTINCT speed FROM ${`mod_tires_sizes`} ORDER BY speed`, function (err: string, result: TireSizesDataResponceDto[]) {
                        if (err) {
                            connection.release();
                            reject(err);
                        }
                        ...
                        connection.execute(`SELECT DISTINCT weight FROM ${`mod_tires_sizes`} ORDER BY weight`, function (err: string, result: TireSizesDataResponceDto[]) {
                            if (err) {
                                connection.release();
                                reject(err);
                            }
                            ...
                            connection.execute(`SELECT DISTINCT brand FROM ${`mod_tires_prices`} ORDER BY brand`, function (err: string, result: TireSizesDataResponceDto[]) {
                                if (err) {
                                    connection.release();
                                    reject(err);
                                }
                                ...
                                connection.execute(`SELECT DISTINCT price_uah FROM ${`mod_tires_prices`} ORDER BY price_uah`, function (err: string, result: TireSizesDataResponceDto[]) {
                                    if (err) {
                                        connection.release();
                                        reject(err);
                                    }
...
export async function createOrder(orderData: OrderItemDto[]) {
    return new Promise( (resolve, reject) => {
        db.getConnection((err, connection) => {
            connection.query('SELECT id FROM `mod_orders_content`', function(err: string, result: OrderDataResponceDto[]) {
                if (err) throw err;
                let id = result[result.length - 1].id;
                connection.query('SELECT id FROM `mod_orders`', function(err: string, result: OrderDataResponceDto[]) {
                    if (err) throw err;
...
async function addNewOrder(orderItems: OrderItemDto[], id: number, orderId: number) {
